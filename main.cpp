...
static DWORD eax_values[] = {
        0x0,        0x1,        0x2,        0x3,        0x4,
        0x5,        0x80000000, 0x80000001, 0x80000002, 0x80000003,
        0x80000004, 0x80000005, 0x80000006, 0x80000007,0x80000008,
};
...
static BOOL dump_cpuid_results(const char *file_path) {
  HANDLE file = CreateFile(file_path, GENERIC_WRITE, FILE_SHARE_READ, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, nullptr);
  ...
  for (auto i = 0; i < num_modes; ++i) {
    ...
    snprintf(output_buffer, 255, "MODE=0x%08X, EAX=0x%08X, EBX=0x%08X, ECX=0x%08X, EDX=0x%08X\n", mode, EAX, EBX, ECX, EDX);
    ...
    if (bytes_written != buffer_len) {
      snprintf(output_buffer, 255, "Partial write failure: wrote %d of %d", bytes_written, buffer_len);
      DbgPrint(output_buffer);
      ret = FALSE;
      goto cleanup;
    }
